...
const renderers = {
        ...
        return `
            ...
            <div class="space-y-6">
                ...
                <div class="pt-6 border-t border-gray-100">
                    ...
                    <p class="text-xs text-gray-500 mb-4">Select what happens after your loved one reads the letter.</p>
                    
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <!-- Choice: Let them decide -->
                        <label class="finale-option cursor-pointer ${pageData.finaleChoice === 'choice' || !pageData.finaleChoice ? 'active' : ''}">
                            <input type="radio" name="finale-choice" value="choice" class="sr-only" 
                                ${pageData.finaleChoice === 'choice' || !pageData.finaleChoice ? 'checked' : ''}
                                onchange="renderers.updateLetter('finaleChoice', this.value)">
                            <div class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 ${pageData.finaleChoice === 'choice' || !pageData.finaleChoice ? 'border-amber-400 bg-amber-50' : 'border-gray-200 bg-white hover:border-amber-200'} transition-all">
                                <span class="material-symbols-outlined text-2xl ${pageData.finaleChoice === 'choice' || !pageData.finaleChoice ? 'text-amber-500' : 'text-gray-400'}">route</span>
                                <div class="text-center">
                                    <div class="font-bold text-xs text-gray-800">Let Them Choose</div>
                                    <div class="text-[10px] text-gray-500">Show all options</div>
                                </div>
                            </div>
                        </label>

                        <!-- Love Lock -->
                        <label class="finale-option cursor-pointer ${pageData.finaleChoice === 'lock' ? 'active' : ''}">
                            <input type="radio" name="finale-choice" value="lock" class="sr-only" 
                                ${pageData.finaleChoice === 'lock' ? 'checked' : ''}
                                onchange="renderers.updateLetter('finaleChoice', this.value)">
                            <div class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 ${pageData.finaleChoice === 'lock' ? 'border-purple-400 bg-purple-50' : 'border-gray-200 bg-white hover:border-purple-200'} transition-all">
                                <span class="material-symbols-outlined text-2xl ${pageData.finaleChoice === 'lock' ? 'text-purple-500' : 'text-gray-400'}">lock_person</span>
                                <div class="text-center">
                                    <div class="font-bold text-xs text-gray-800">Love Lock</div>
                                    <div class="text-[10px] text-gray-500">Lock your love</div>
                                </div>
                            </div>
                        </label>

                        <!-- Infinity Scroll -->
                        <label class="finale-option cursor-pointer ${pageData.finaleChoice === 'infinity' ? 'active' : ''}">
                            <input type="radio" name="finale-choice" value="infinity" class="sr-only" 
                                ${pageData.finaleChoice === 'infinity' ? 'checked' : ''}
                                onchange="renderers.updateLetter('finaleChoice', this.value)">
                            <div class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 ${pageData.finaleChoice === 'infinity' ? 'border-blue-400 bg-blue-50' : 'border-gray-200 bg-white hover:border-blue-200'} transition-all">
                                <span class="material-symbols-outlined text-2xl ${pageData.finaleChoice === 'infinity' ? 'text-blue-500' : 'text-gray-400'}">all_inclusive</span>
                                <div class="text-center">
                                    <div class="font-bold text-xs text-gray-800">Infinity</div>
                                    <div class="text-[10px] text-gray-500">Endless memories</div>
                                </div>
                            </div>
                        </label>

                        <!-- Valentine Invitation -->
                        <label class="finale-option cursor-pointer ${pageData.finaleChoice === 'invitation' ? 'active' : ''}">
                            <input type="radio" name="finale-choice" value="invitation" class="sr-only" 
                                ${pageData.finaleChoice === 'invitation' ? 'checked' : ''}
                                onchange="renderers.updateLetter('finaleChoice', this.value)">
                            <div class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 ${pageData.finaleChoice === 'invitation' ? 'border-pink-400 bg-pink-50' : 'border-gray-200 bg-white hover:border-pink-200'} transition-all">
                                <span class="material-symbols-outlined text-2xl ${pageData.finaleChoice === 'invitation' ? 'text-pink-500' : 'text-gray-400'}">favorite_border</span>
                                <div class="text-center">
                                    <div class="font-bold text-xs text-gray-800">Invitation</div>
                                    <div class="text-[10px] text-gray-500">Ask them out</div>
                                </div>
                            </div>
                        </label>
                    </div>
    ...
    initQuill() {
        ...
        // ✅ FIX: Also sync on blur to ensure content is saved when user clicks away
        this.quill.root.addEventListener('blur', () => {
            const html = this.quill.root.innerHTML;
            console.log('[Quill] Blur - syncing content, length:', html.length);
            this.updateLetter('message', html);
        });
    },
